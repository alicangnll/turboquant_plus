...
struct llama_context_params common_context_params_to_llama(const common_params & params) {
    ...
    cparams.turbo_async      = params.turbo_async;
    cparams.sparse_v_thresh  = params.sparse_v_thresh;

    // Sparse-V threshold is currently also read from env var in ggml-metal
    // we set it here to ensure consistency if changed via CLI
    if (params.sparse_v_thresh >= 0) {
        char buf[32];
        snprintf(buf, sizeof(buf), "%f", params.sparse_v_thresh);
        setenv("TURBO_SPARSE_V_THRESHOLD", buf, 1);
    }
